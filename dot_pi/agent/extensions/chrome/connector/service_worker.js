...
async function dispatch(action, params) {
  switch (action) {
    ...
    case "browser.download":
      return browserDownload(params);
    case "browser.saveMhtml":
      return browserSaveMhtml(params);
    case "browser.cookies":
      return browserCookies(params);
...
//     inspect, evaluate, waitFor, console/network list, probe. These need a live
...
// ---------------------------------------------------------------------------
// Browser-level utilities (no tab required). Backed by manifest permissions:
// downloads, pageCapture, cookies.
// ---------------------------------------------------------------------------

async function browserDownload(params) {
  const options = { url: String(params.url) };
  if (params.filename) options.filename = String(params.filename);
  if (params.saveAs === true) options.saveAs = true;
  const downloadId = await chrome.downloads.download(options);
  return { downloadId };
}

async function browserSaveMhtml(params) {
  const tab = await getTabByParams(params);
  const dataUrl = await chrome.pageCapture.saveAsMHTML({ tabId: tab.id });
  return { tab: await formatTab(tab), mhtml: dataUrl };
}

async function browserCookies(params) {
